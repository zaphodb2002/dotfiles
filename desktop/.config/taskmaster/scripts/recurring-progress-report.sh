...
BELONGING_DOGS=( "df2601bc" "f16a4d21" "b901bef7" "8aeea8d6" "2123be20" "fd05d761" "1da4b47c" "69904e9a" "f2c4855f" "a460d48a" "909ac4c9" "acad1617" "f8d3c164" )

RECREATION_PSYCHONAUTICS_CANNABIS=( "142ab3a9" )

report () {
	cd $HOME/.config/taskmaster/scripts
	if [[ "$1" = "lms" ]]; then
	...
	if [ "$1" = "belonging" ]; then
		echo "Belonging.Dogs"
		LEN=$((${#BELONGING_DOGS[@]} - 1))
		for i in $(seq 0 ${LEN}); do
			project-progress.sh ${BELONGING_DOGS[$i]}
		done
	fi

	if [ "$1" = "recreation" ]; then
		echo "Recreation.Psychonautics.Cannabis"
		LEN=$((${#RECREATION_PSYCHONAUTICS_CANNABIS[@]} - 1))
		for i in $(seq 0 ${LEN}); do
			project-progress.sh ${RECREATION_PSYCHONAUTICS_CANNABIS[$i]}
		done
	fi
}
